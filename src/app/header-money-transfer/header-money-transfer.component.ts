...
export class HeaderMoneyTransferComponent implements OnInit, OnDestroy {
  ...
  confirmTransfer() {
    const transferData = {
      accountDTO: {
        ...
        accountHolderName: this.recipientName,
      },
      sendingCurrency: this.sendCurrency,
      receivingCurrency: this.receiveCurrency,
      sentAmount: this.sendAmount,
    };

    this.authService.makeTransfer(transferData).subscribe(
      (response) => {
        console.log('Transfer successful:', response);
        this.goToNextStep(); // Move to the next step after a successful transfer
      },
      (error) => {
  ...
  addToFavourites() {
    ...
    this.favourites = favourites; // Update the local array to reflect changes
  }
  ...
  deleteFavourite(accountNumber: string) {
